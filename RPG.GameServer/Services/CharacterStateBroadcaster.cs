...
public class CharacterStateBroadcaster : ICharacterStateBroadcaster
{
    ...
    public Task BroadcastAsync(CharacterStateUpdate update, CancellationToken cancellationToken = default)
    {
        if (update == null)
        {
            _logger.LogWarning("Received null character state update.");
        ...
        _logger.LogDebug("Updated state for character {CharacterId} | moving={IsMoving} rotating={IsRotating} rotation={Rotation}",
